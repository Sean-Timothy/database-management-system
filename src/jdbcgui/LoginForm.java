...
public class LoginForm extends javax.swing.JFrame {
    ...
    public LoginForm() {
        ...
        try {            
            rs = dbCon.executeStatement("SELECT Username, Password, UserType FROM USER_AIRPORT ORDER BY username" );
    ...
    private void btnLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLoginActionPerformed
        ...
        // TODO add your handling code here:
            try {
                ...
                while (rs.next()) {
                    /*
                    String storedEncryptedPass = rs.getString("encryptedpass");
                    if (storedEncryptedPass == null) {
                        // The stored encrypted password is NULL
                        // Update the record with the new encrypted password
                        String newEncryptedPass = hashPassword(txtPassword.getText().trim());
                        rs.updateString("encryptedpass", newEncryptedPass);
                        rs.updateRow();
                        storedEncryptedPass = newEncryptedPass;
                    }*/

                    //String hashedPassword = hashPassword(txtPassword.getText().trim());
                    //&& storedEncryptedPass.equals(hashedPassword)
                    if (rs.getString("Username").equals(txtUsername.getText().trim())) {
                        // Valid Login User found
                        validLogin = true;
                        // create a new login user object and pass details to Menu form
                        (new Menu(new LoginUser(rs.getString("Username"),rs.getString("Password"), rs.getInt("UserType")))).setVisible(true);
                        this.dispose(); // remove this login form from memory
                    }
                }
